...
class FabricDeploymentPipeline:
    ...
    def deploy_to_stage(self, source_stage_order: int, target_stage_order: int, 
                        note: Optional[str] = None, wait: bool = True) -> bool:
        """
    ...
    def _wait_for_deployment(self, operation_id: str, target_stage: str, 
...
def main():
    ...
    parser.add_argument(
        "--pipeline-id",
        required=True,
        help="Fabric Deployment Pipeline ID"
    )
    parser.add_argument(
        "--source-stage",
        type=int,
    ...
    parser.add_argument(
        "--info",
        action="store_true",
        help="Just show pipeline info, don't deploy"
    )
