...
class NotebookValidator:
    ...
    def check_cells(self, path: Path, nb: Any) -> bool:
        ...
        if markdown_cells == 0:
            self.warnings.append(f"{path.name}: No markdown cells (consider adding documentation)")
    ...
    def check_outputs(self, path: Path, nb: Any) -> bool:
        ...
        if cells_with_output > 0:
            ...
            print(f"  ⚠️  Outputs: {cells_with_output} cell(s) have outputs (consider clearing)")
        else:
