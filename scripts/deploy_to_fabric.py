    ...
    def _deploy_notebook(self, notebook_path: Path) -> bool:
        ...
        try:
            ...
            if existing_notebook:
                ...
                success = self._update_notebook(notebook_id, notebook_name, notebook_content)
            else:
...
def main():
    ...
    parser.add_argument("--environment", required=True, choices=["dev", "test", "prod"], help="Target environment")
    parser.add_argument("--artifact-type", required=True, choices=["notebooks", "pipelines", "all"], help="Type of artifacts to deploy")
    parser.add_argument("--artifacts-path", default=".", help="Path to artifacts directory")
    parser.add_argument("--validate-before-deploy", action="store_true", help="Validate workspace before deployment")
